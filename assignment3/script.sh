...
# Selezione passi
PASS_OPTIONS=("loop-pass" "custom-LICM-pass" "tutti")
...
case $selected_pass in
    "tutti") PASSES="loop-pass,custom-LICM-pass" ;;
...
if [ ${C_EXEC} == true ]; then
    echo -e "\nGenerazione IR...\n"
    clang -Xclang -disable-O0-optnone  -fno-discard-value-names  -O0 -S -emit-llvm -c "${TEST_FILE_PATH}.${TEST_FILE_EXTENSION}" -o "${TEST_FILE_PATH}.ll" # added -fno-discard-value-names to avoid name mangling
